...
$ttl_tblgho_feature_post 	= $fields['ttl_tblgho_feature_post'];
$ttl_author_avatar 		= $fields['ttl_author_avatar'];
$ttl_pagetitle = (isset($fields['ttl_tblgho_title']) && $fields['ttl_tblgho_title']!='' ) ? $fields['ttl_tblgho_title'] : get_the_title();
...
<div class="blog-hero">
	<div class="wrapper">
		<div class="banner-content">
			<h1><?php echo $ttl_pagetitle; ?></h1>
		</div>
		<div class="s-50"></div>
		<div class="blog-nav">
			<div class="nav-ctn d-flex justify-content-center flex-wrap">
				<?php if($ttl_post_catagories){ ?>
					<ul>
						<?php foreach ($ttl_post_catagories as $category ) { ?>
		...
		<?php
		global $post;
		if($ttl_tblgho_feature_post){ ?> <?php foreach ($ttl_tblgho_feature_post as $key => $feature_post) {
			$post = $feature_post;
			setup_postdata( $post );
			$pID         = $post->ID;
			...
			$author_avatar = ($ttl_author_avatar)? $ttl_author_avatar : get_avatar_url($pID);
